...
test_that("get_github_by_topic_graphql() errors if topics is empty", {
  skip_on_cran()
  skip_on_ci()
  expect_error(get_github_by_topic_graphql(character(), token = gitcreds::gitcreds_get()$password), "At least one topic must be provided")
})
...
test_that("get_github_by_topic_graphql() returns a data frame", {
  ...
  result <- get_github_by_topic_graphql(c("rstats"), token = gitcreds::gitcreds_get()$password, limit = 5)
...
test_that("Returned data frame contains expected columns", {
  ...
  result <- get_github_by_topic_graphql(c("bioinformatics"), token = gitcreds::gitcreds_get()$password, limit = 3)
  expected_cols <- c(
...
test_that("get_github_by_topic_graphql() returns no results for nonsense topic", {
  ...
  result <- get_github_by_topic_graphql(c("thisisaveryunlikelytopicname1234567890"), token = gitcreds::gitcreds_get()$password)
  expect_s3_class(result, "data.frame")
